...
//
//needs some work
// User needs to hard-code the foil positions (see lines 211) and old matrix element filename.
// Read the cout to make sure it's reading the right matrix elements
void fit_opt_matrix(TString fname="Optics_1544_-1_fit_tree2") {
  gROOT->Reset();
  gStyle->SetOptStat(0);
 ...
 gStyle->SetPadLeftMargin(0.17);
  ...
  string newcoeffsfilename="nps_hms_newfit2_all_1544_-10.dat";
//   string oldcoeffsfilename="hms_recon_coeff_opt2018.dat";
 ...
 TH1F *hxptardiff = new TH1F("hxptardiff","xptar diff (mr)",100,-25,25);
 TH1F *hxptarnew = new TH1F("hxptarnew","xptar new recon",100,-.12,.12);
 TH1F *hxptarnewdiff = new TH1F("hxptarnewdiff","xptar new diff (mr)",100,-25,25);
 TH1F *hyptar = new TH1F("hyptar","yptar ",100,-.1,.1);
 TH1F *hyptarrecon = new TH1F("hyptarrecon","yptar recon ",100,-.1,.1);
 TH1F *hyptardiff = new TH1F("hyptardiff","yptar diff (mr) ",100,-30,30);
 TH1F *hyptarnew = new TH1F("hyptarnew","yptar new recon ",100,-.1,.1);
 TH1F *hyptarnewdiff = new TH1F("hyptarnewdiff","yptar new diff (mr) ",100,-30,30);
  //
...
//Declaration of leaves types
 ...
 const Int_t nfoils=1;
// Below, enter the coordinates of the foils
  vector <Double_t> ztar_foil;
   // ztar_foil.push_back(8.);
...
//now the delta cuts
	...
	Double_t delcut[ndelcut+1]={-10,-8.,-5,0.,5.,10.};
