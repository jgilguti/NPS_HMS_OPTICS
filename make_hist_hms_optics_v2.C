...
// Edit line 271 for new matrix elements
// make your matrix header match one that works, or else this won't work
// Verify input and output root files are correct
void make_hist_hms_optics_v2(Int_t nrun=4819,Bool_t CutYtarFlag=kTRUE,Bool_t CutYpFpYFpFlag=kTRUE,Bool_t CutXpFpXFpFlag=kTRUE,Int_t FileID=-2){
...
//   tsimc->SetBranchAddress("H.hgcer.npeSum",&sumhgnpe);
 ...
 Double_t frx;
   ...
   tsimc->SetBranchAddress("H.rb.raster.fr_ya",&fry);
	...
	HList.Add(hztar);
//Just added ztarg below.  I don't see where ztar (above) is located in the rest of the script.
...
//Double_t yMP = 0.1*(0.52-0.012*40.0+0.002*40.*40.);
Double_t yMP  = 0.1*(0.52-0.012*CentAngle+0.002*CentAngle*CentAngle);

/*
 if (CentAngle<40.0){
	yMP = 0.1*(0.52-0.012*CentAngle+0.002*CentAngle*CentAngle);
	}
*/
cout << "CentAngle = " << CentAngle << endl;
cout << "yMP = " << yMP << endl;
cout << "y_mis = " << y_mis << endl;

//CentAngle *= 3.14/180.;//this used to be in a loop.  every loop this tries to convert again, I think, so I commented out
for (int i = 0; i < nentries; i++) {
  ...
  hybpm_tar->Fill(ybpm_tar);
    ...
    if (sumnpe > 2. && delta>-10 && delta<10) {
      //reconstruct from the matrix elements on the fly:
	...
	xsieve = xtar + xptartemp*168.0;
	Double_t delta_per = deltatemp*100.0;
	
	ysieve = ytartemp*100+yptartemp*168.;//not 100% sure on this one, but it's a guess
	ytartemp *=100;
  		
	Double_t ztarCalc = (ytar+y_mis) + xbpm_tar*(costh-yptar*sinth);
	ztarCalc=-ztarCalc/(sinth+yptar*costh);
	Double_t ztarg=-(ytartemp+yMP+reactx*(costh-yptartemp*sinth))/(sinth+costh*yptartemp);//updated 9/5 but needs to be verified by papers
	//Double_t ztarg=(ytartemp-yMP-reactx*(cos(CentAngle)+yptartemp*sin(CentAngle)))/(sin(CentAngle)-cos(CentAngle)*yptartemp);//verify
	/*  if (delta>-10 && delta<10) hytar->Fill(ytar);
	  if (delta>-10 && delta<10) hztar->Fill(reactz);
	  if (delta>-10 && delta<10) hztarg->Fill(ztarg);
	...
	*/
	...
	hztarCalc->Fill(ztarCalc);  
      //plot the stuff from these matrix elements
	...
	//cout << "just filled hZtarDelta " << endl;
	//cout << "what is ytar_delta_cut.size()? " << ytar_delta_cut.size() << endl;

	  for  (UInt_t ng=0;ng<ytar_delta_cut.size();ng++) {
	    //cout << "ng = " << ng << endl;
	    if (ytar_delta_cut[ng]->IsInside(ytar,delta))	{ 
	      //cout << "Filling sieve, delta " << endl;
	      hYsDelta[ng]->Fill(ysieve,delta);
	      hXsDelta[ng]->Fill(xsieve,delta);
	      hYpFpYFp[ng]->Fill(ypfp,yfp);
	      hXFpYFp[ng]->Fill(yfp,xfp);
	      hXpFpXFp[ng]->Fill(xpfp,xfp);
              for  (Int_t nd=0;nd<ndelcut;nd++) {
	        if ( delta >=delcut[nd] && delta <delcut[nd+1]) {
                  hYsXs_DelCut[ng][nd]->Fill(ysieve,xsieve); 
	          hYpFpYFp_DelCut[ng][nd]->Fill(ypfp,yfp);
	          Int_t f_ny=-1;
                  for  (UInt_t ny=0;ny<9;ny++) {
		     if (CutYpFpYFpFlag && ypfp_yfp_cut[ng][nd][ny] && ypfp_yfp_cut[ng][nd][ny]->IsInside(ypfp,yfp)) {
		       hYsXs_DelCut_YpYfpCut[ng][nd][ny]->Fill(ysieve,xsieve);
		       hXs_DelCut_YpYfpCut[ng][nd][ny]->Fill(xsieve);
		       f_ny=ny;
		       }
		     }//end ny loop
		  // temporary to clean up
		   if (f_ny !=-1) hXpFpXFp_DelCut[ng][nd]->Fill(xpfp,xfp);
		   hXpFpXFp_DelCut[ng][nd]->Fill(xpfp,xfp);
                   for  (UInt_t nx=0;nx<9;nx++) {
		     //cout << "nx loop " << endl;
		     if (f_ny !=-1 && CutXpFpXFpFlag && xpfp_xfp_cut[ng][nd][nx] && xpfp_xfp_cut[ng][nd][nx]->IsInside(xpfp,xfp)) {
		       hYsXs_DelCut_XpXfpCut[ng][nd][nx]->Fill(ysieve,xsieve);
		       }
		   }//nx loop
		}//if(delta cuts)			     
	      }//nd loop
	    }//if(IsInside)
//cout << " inside the cursed ng loop " << endl;
	  }//ng loop
//cout << " inside the sumnpe delta loop " << endl;
	}//sumnpe, delta loop, beginning of matrix recon
      }
