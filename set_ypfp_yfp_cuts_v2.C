...
void set_ypfp_yfp_cuts_v2(Int_t nrun=1814,Int_t FileID=-2,Double_t hist_minZ=1.) {
 ...
 //
  ...
  //------------------ Flag 1 ----> Set the Polygon Cut ----------------------
	...
	while( nd!=-1 ) {
	  ...
	  cin >> nd;
	  if (nd !=-1)cout << " Pick minimum number of events per bin" << " hist_minZ = " << hist_minZ << endl;
	  if (nd !=-1)cin >> hist_minZ;
	  if (nd !=-1&&hist_minZ==0) hist_minZ+=1;
          if (nd >=ndelcut) nd=0;
	...
	fhistroot->cd();
	 ...
	 //       if(!t){cout << "No cut  =  " <<hname_cut  << endl;}
	  ...
	  cout <<" Action ( 0 ( set cut) , -1 (pick next nd and foil or change minZ), -10 delete cut) "  << endl;
          cin >> nloop ;
	  if (nloop == -100) return;
...
// this chunk works where it is, but trying to move to after foil choice but before sieve hole choice
		  Double_t minx=0;
		  Double_t miny=0;
		  Double_t maxx=0;
		  Double_t maxy=0;
	 ...
	 fhistroot->cd();
		 ...
		 gPad->SetLogz();    
        	 hYpFpYFp_DelCut[nf][nd]->Draw("colz");
        	 hYpFpYFp_DelCut[nf][nd]->SetMinimum(hist_minZ); 
        	 hYpFpYFp_DelCut[nf][nd]->GetYaxis()->SetRangeUser(miny,maxy);
       	 	 hYpFpYFp_DelCut[nf][nd]->GetXaxis()->SetRangeUser(minx,maxx);
                 histView_Cut->Update();
          fcut.cd();
                  for  (Int_t nys=0;nys<9;nys++) {
        	    hname_cut= Form("hYpFpYFp_cut_yscol_%d_nfoil_%d_ndel_%d;1",nys,nf,nd);
